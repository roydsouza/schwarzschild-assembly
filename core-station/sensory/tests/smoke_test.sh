...
echo ""
echo "=== Aethereum-Spine Phase 1 Smoke Test ==="
echo ""
  ...
  "resourceMetrics": [
    {
      "resource": {
        "attributes": [
          { "key": "service.name", "value": { "stringValue": "aethereum-spine-smoke-test" } },
          { "key": "service.namespace", "value": { "stringValue": "aethereum-spine" } },
      ...
      "scopeMetrics": [
        {
          "scope": {
            "name": "aethereum-spine.smoke-test",
            "version": "1.0.0"
          },
          "metrics": [
            {
              "name": "aethereum_spine.fitness.safety_compliance",
            ...
            {
              "name": "aethereum_spine.fitness.dhamma_alignment",
...
if [[ "$OTEL_RUNNING" == true ]]; then
  ...
  if [[ ! -f "$LATEST" ]]; then
    ...
    if python3 -c "import json, sys; json.loads(sys.stdin.read())" <<< "$LAST_LINE" 2>/dev/null || \
      ...
      if grep -q "aethereum_spine.fitness" "$LATEST" 2>/dev/null; then
        ok "otel-snapshots/latest.json: contains aethereum_spine.fitness metrics"
      else
        warn "otel-snapshots/latest.json: aethereum_spine.fitness metrics not yet visible (collector may need more time)"
      fi
    else
...
if command -v protoc &>/dev/null; then
  ...
  if protoc --proto_path=aethereum-spine/proto aethereum-spine/proto/orchestrator.proto \
    -o /dev/null 2>/dev/null; then
    ok "aethereum-spine/proto/orchestrator.proto: valid"
  else
    fail "aethereum-spine/proto/orchestrator.proto: protoc parse error"
  fi
else
