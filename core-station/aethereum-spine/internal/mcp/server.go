...
func (h *Host) handleInitialize() interface{} {
	return map[string]interface{}{
		...
		"serverInfo": map[string]string{
			"name":    "aethereum-spine-aethereum-spine",
...
func (h *Host) callApproveAction(ctx context.Context, args json.RawMessage) (*CallToolResult, error) {
	...
	if isSec {
		...
		return &CallToolResult{
			IsError: true,
			Content: []Content{{Type: "text", Text: "Error: security-adjacent proposals cannot be approved via MCP. Use the Aethereum-Spine Control Panel for human signature verification."}},
		}, nil
	}
