...
#
# Called automatically by the root-spine on a scheduled interval, and manually
...
echo "checkpoint.sh: root-spine merkle checkpoint"
...
if [[ "${1:-}" == "--verify-only" ]]; then
  ...
  echo "The root-spine internal/merkle package will implement this."
  exit 0
fi
